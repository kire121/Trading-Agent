...
from research.hashutil import compute_config_hash  # noqa: E402
...
def _compare(committed: dict, rerun: dict):
    ...
    lines = []
    all_keys = sorted(set(committed_flat) | set(rerun_flat))
    passed = 0
    for key in all_keys:
        ref = committed_flat.get(key, "<saknas>")
        new = rerun_flat.get(key, "<saknas>")
        ...
        if ok:
            passed += 1
        lines.append(f"[{'PASS' if ok else 'FAIL'}] {key} = {new} (referens: {ref})")
...
def main():
    ...
    if not matches:
        ...
        sys.exit(2)
    if len(matches) > 1:
        print(f"VARNING: {len(matches)} frysta configs matchar hash {args.config_hash}, "
              f"använder första: {matches[0][0]}", file=sys.stderr)
    ...
    was_oos_unlocked = committed.get("data_window", {}).get("oos_unlocked", False)
    rerun = compute_results(config, unlock_oos=was_oos_unlocked)
