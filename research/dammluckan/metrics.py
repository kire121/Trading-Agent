"""
Dammluckan -- performance/risk statistics.

DSR/PSR follow the Bailey & Lopez de Prado (2014) formulation, which is
copy-structurally identical across every sibling research branch in this
repo (fasflocken/stats.py, oglegrinden/stats.py, irreversibility_lab/
validation.py, formdriften/metrics.py) -- reproduced here in the same form.
Newey-West t-stats use statsmodels HAC, matching oglegrinden/formdriften.
"""
import numpy as np
import pandas as pd
...
def deflated_sharpe_ratio(sr_hat: float, n_obs: int, sr_trials, skew: float = 0.0,
    ...
    """Returns both the DSR probability (PSR against the expected-max-under-
    the-null benchmark) and the Sharpe-scaled excess gap (observed -
    E[max under null]), since a bare "DSR <= 0" rule needs the gap, not the
    probability. sr_hat and sr_trials must both be per-period Sharpes."""
...
def pnl_quarter_concentration(returns: pd.Series) -> float:
    """Max fraction of total |PnL| (sum of returns) from a single calendar
    quarter -- ported from formdriften/metrics.py::pnl_quarter_concentration."""
    r = returns.dropna()
    if not len(r):
        return np.nan
    q = r.groupby([r.index.year, r.index.quarter]).sum()
    total = q.abs().sum()
    return float(q.abs().max() / total) if total != 0 else np.nan
...
def sub_period_sign_consistency(returns: pd.Series, start, end, n_periods=config.N_SUBPERIODS) -> dict:
    ...
    sub-periods; kill criterion is inconsistency in >= 2 of n_periods
    relative to the full-sample sign."""
    ...
    n_inconsistent = sum(1 for s in signs if s != 0 and s != full_sign)
    return {"full_sign": full_sign, "period_signs": signs, "period_ann_returns": ann_rets,
            "n_inconsistent": n_inconsistent, "boundaries": boundaries}


def event_concentration(trades, tickers) -> dict:
    """Steg 0: per-side event-count and max-single-asset-share check."""
    from collections import Counter
    counts = Counter(t.ticker for t in trades)
    total = sum(counts.values())
    max_share = (max(counts.values()) / total) if total else np.nan
    max_ticker = max(counts, key=counts.get) if counts else None
    full_counts = {t: counts.get(t, 0) for t in tickers}
    return {"n_events": total, "counts": full_counts, "max_share": max_share, "max_ticker": max_ticker}
