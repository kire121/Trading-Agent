...
Ported from research/runraden/eodhd_client.py (same API surface, same
caching convention). Only fetches adjusted daily OHLCV. No point-in-time
survivorship handling is attempted beyond what EODHD serves for the ticker
list supplied by the caller -- see config.py's "registerluckan" caveat.
"""
...
def _api_key() -> str:
    key = os.environ.get(config.EODHD_API_KEY_ENV)
    if not key:
        raise EODHDError(f"{config.EODHD_API_KEY_ENV} not set in environment")
    return key
