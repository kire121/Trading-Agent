# Proveniens: research/timglaset/tests/test_opclock.py, branch
# claude/strategy-spec-implementation-axn5co, commit 408c81f. Flyttad till
# tests/ tillsammans med lib/opclock.py vid lib-konsolideringen
# (docs/INSTRUKTION.md avsnitt 7). Endast importvägen
# (research.timglaset.opclock -> lib.opclock) och de nu obligatoriska
# window/cap-argumenten till compute_tau (tidigare Timglaset-specifika
# defaultvärden, se lib/opclock.py) är ändrade -- testfallen och deras
# toleranser är annars oförändrade.
"""Clock-machinery test cases 1-6, covering compute_tau, compute_op_ewma,
compute_raw_z and compute_signal (the three public API functions plus
their shared burn-in helper). Cases 7-9 in the original Timglaset test
suite (end-to-end synthetic generator, liveness-assertion regression,
DSR-tiling reproduction) required strategy-specific twins.py/ladder.py
machinery and were not part of the generic clock API, so they were not
migrated here -- they remain in research/timglaset/tests/test_integration.py
on the frozen strategy branch.
"""
import sys
...
from lib import opclock  # noqa: E402
...
# ---------------------------------------------------------------------------
# 2. Scale invariance: activity x10 globally -> tau unchanged after burn-in.
# ---------------------------------------------------------------------------
def test_2_tau_scale_invariance():
    rng = np.random.default_rng(1)
    n = 400
    vol = rng.lognormal(mean=10, sigma=0.3, size=(n, 3))
    activity = _mkdf(vol, cols=["a", "b", "c"])
    tau1 = opclock.compute_tau(activity, window=252, cap=5.0)
    tau10 = opclock.compute_tau(activity * 10.0, window=252, cap=5.0)
...
def test_4_pit_no_lookahead():
    ...
    activity = _mkdf(vol, cols=["x", "y"])
    returns = _mkdf(ret, cols=["x", "y"])

    tau_full = opclock.compute_tau(activity, window=252, cap=5.0)
    ...
    cutoff = 350
    activity_trunc = activity.iloc[:cutoff]
    returns_trunc = returns.iloc[:cutoff]
    tau_trunc = opclock.compute_tau(activity_trunc, window=252, cap=5.0)
...
# ---------------------------------------------------------------------------
# 5. NaN-/zero-activity day -> tau=1 fallback path, no NaN propagation in M/V.
# ---------------------------------------------------------------------------
def test_5_nan_and_zero_activity_no_propagation():
    ...
    tau = opclock.compute_tau(activity, window=252, cap=5.0)
    assert tau.iloc[300, 0] == pytest.approx(1.0)  # NaN activity -> fallback 1.0
    assert tau.iloc[310, 0] == pytest.approx(1.0)  # zero activity -> fallback 1.0
...
def test_5b_zero_activity_is_isolated_not_propagating():
    """A zero-activity day is a real (non-null) numerator value, so unlike a
    NaN it must NOT poison the rolling-median window for ~`window` following
    days -- only the single zero-activity day itself falls back to tau=1."""
    ...
    # continuous lognormal activity, an exact ratio of 1.0 has probability 0.
...
# ---------------------------------------------------------------------------
# 7. variance_clock_input as an activity proxy: feeding it through
#    compute_tau/compute_op_ewma exercises the "arbitrary activity proxy"
#    contract end to end (not just volume), matching the generalization
#    made at the research/ -> lib/ promotion.
# ---------------------------------------------------------------------------
def test_7_variance_clock_input_as_activity_proxy():
    rng = np.random.default_rng(6)
    n = 400
    ret = rng.normal(0, 0.01, size=(n, 2))
    returns = _mkdf(ret, cols=["x", "y"])

    activity = opclock.variance_clock_input(returns, window=5)
    tau = opclock.compute_tau(activity, window=252, cap=5.0)
    assert not tau.isna().any().any()
    M, V, T = opclock.compute_op_ewma(returns, tau, 63.0)
    z = opclock.compute_signal(M, V, T, 63.0, "clip2")
    assert np.isfinite(z.to_numpy()[~np.isnan(z.to_numpy())]).all()
